...
/* Sin respaldo aqui a proposito -- son credenciales reales, no algo
   que deba quedar escrito en el repo. Confirmado en Vercel
   (21/09/2026) que las 3 ya estan puestas en production, preview y
   development, asi que el respaldo ya no hacia falta. */
...
const ORDER_SEEN_BY_LIST = 'OrderSeenBy'; // OrderID, ViewerId, SeenAt -- ver gsocd-shared/lib/seen-tracking.js
...
/* "Assign by service" (21/09/2026) -- una fila por servicio-en-una-
   orden-en-modo-por-servicio: OrderID+Category+ServiceName la liga a
   su renglon real en OrderServices (mismo par que ya usa el resto
   del sistema para identificar un servicio dentro de una orden).
   AssignedTo es texto (nombres separados por coma), igual que
   Supervisor en Orders -- no es un campo de tipo Persona. */
...
const SETTINGS_LIST = 'Settings';
const FIELD_EMPLOYEES_LIST = 'FieldEmployees';
const SCHEDULING_LIST = 'Scheduling';
const WEEKLY_HOURS_LIST = 'WeeklyHours';
const REPORT_UPLOADS_LIST = 'ReportUploads';
...
const RECURRING_IMPORT_MATRIX_LIST = 'RecurringImportMatrix';
...
/* Suscripciones de notificaciones push -- lista compartida con
   tech.gsocd.com (mismo sitio de SharePoint). tech.gsocd.com las crea
   cuando el tecnico da permiso de notificaciones; Admin es quien las
   lee para mandar el push (aqui es donde pasan los cambios de orden). */
const PUSH_SUBSCRIPTIONS_LIST = 'PushSubscriptions';

/* Holidays: lista maestra (uno solo, compartida por todos los
   clientes como opcion) -- confirmado con el usuario, se busca en
   linea la lista de festivos reconocidos en EEUU y se precargan.
   RuleType 'Fixed' usa Month+Day tal cual; 'NthWeekday' calcula la
   fecha real cada año (Nth: 1-4, o 5 para "ultimo"; Weekday: 0=Sunday). */
const HOLIDAYS_LIST = 'Holidays';
/* Eleccion de CADA cliente (y, si aplica, de un building especifico --
   BuildingId vacio significa la direccion principal) sobre un festivo
   de la lista maestra: abre o cierra, y si abre, a que hora. */
const CLIENT_HOLIDAYS_LIST = 'ClientHolidays';
/* Acceso por QR sin login (tech.gsocd.com) -- Admin genera el
   SetupToken desde Techs & Roles, Tech lo reclama y crea el
   DeviceToken permanente. Misma lista de SharePoint que
   tech.gsocd.com/lib/graph.js. */
const TECH_DEVICE_TOKENS_LIST = 'TechDeviceTokens';
/* Registro de cada foto/video subido (quien, cuando, donde) --
   misma lista de SharePoint que tech.gsocd.com/lib/graph.js, para el
   reporte de Times and Routes. */
const TECH_PHOTO_LOG_LIST = 'TechPhotoLog';
  ...
  try {
    ...
    return (data.value || []).map(x => ({
      ...
      /* URL de descarga temporal (pre-autenticada, ~1h) que Graph ya
         ...
         ya aplicado en tech.gsocd.com/lib/graph.js. */
...
/* Nominatim (OpenStreetMap) -- gratis, sin cuenta, sin tarjeta. Nunca
   ...
   null y ya, las coordenadas se pueden rellenar despues con el
   backfill de Developer si esta vez no jalo. Compartido por
   admin-update-client.js (crear/editar building) y developer-admin.js
   (backfill de los que ya existen). */
/* Borra un archivo o carpeta por su id (recursivo si es carpeta --
   asi es como Graph maneja DELETE, de una sola vez). Usado SOLO por
   wipe-test-data (Developer > Debug) para de verdad tirar fotos y
   documentos de ordenes de prueba, no solo las filas de las listas.
   Silencioso si ya no existe (pudo borrarse en una corrida anterior). */
async function deleteDriveItemById(itemId) {
  const driveId = await getDriveId();
  try {
    await graphFetch('/drives/' + driveId + '/items/' + itemId, { method: 'DELETE' }, true);
  } catch (e) { /* ya no existia, o error transitorio -- no es critico para un wipe de prueba */ }
}
...
async function geocodeAddress(address, city, zip) {
  ...
  /* Nunca nos importa la suite/local para geocodificar -- solo
     necesitamos el punto del EDIFICIO (lat/lon), y encima Nominatim
     casi nunca tiene registrada la suite individual, solo el
     edificio completo. Se quita de una vez, sin intentar mandarla
     primero. */
  ...
  /* Intento 1: busqueda ESTRUCTURADA -- mucho mas confiable que un
     solo texto libre, sin ambiguedad de donde termina la calle y
     empieza la ciudad. */
  ...
  /* Intento 2: si la estructurada no encontro nada (direccion
     incompleta, formato raro, etc), texto libre de respaldo -- mejor
     que nada. */
...
module.exports = {
  PATHS,
  CLIENTS_LIST, ORDERS_LIST, ORDER_SERVICES_LIST, ORDER_HISTORY_LIST, ORDER_SEEN_BY_LIST, DRAFTS_LIST,
  CLIENT_HISTORY_LIST, CLIENT_ADDRESSES_LIST, CLIENT_CONTACTS_LIST, ORDERS_FOLDER, SERVICES_LIST, SERVICES_CATALOG_LIST, SERVICE_TEMPLATES_LIST, STAFF_LIST, SETTINGS_LIST,
  ...
  TECHS_LIST, ORDER_ASSIGNMENTS_LIST, SERVICE_TIMES_LIST, CLIENT_PACKAGES_LIST, PUSH_SUBSCRIPTIONS_LIST, SERVICE_ASSIGNMENTS_LIST,
  ...
  findFolderByPrefix, ensureFolder, uploadFile, downloadByPath, deleteDriveItemById,
  geocodeAddress,
  jsonResponse
};
