...
/* ---- Todos los articulos de QuickBooks (26/09/2026, Developer >
   Migrate from QuickBooks): activos e inactivos, para actualizar el
   catalogo de la app por SKU sin tener que bajar y subir el reporte.
   SELECT * porque Sku no viene si se piden campos sueltos (ver
   findItemBySku). Las Category/Group de QuickBooks no son servicios. */
async function listItems() {
  const out = [];
  for (let start = 1; start <= 5001; start += 1000) {
    const data = await qbQuery(`select * from Item where Active in (true, false) startposition ${start} maxresults 1000`);
    const rows = (data.QueryResponse && data.QueryResponse.Item) || [];
    out.push(...rows);
    if (rows.length < 1000) break;
  }
  return out.filter(it => it.Type !== 'Category' && it.Type !== 'Group');
}
...
/* Borrar desde Admin (25/09/2026, tab Sent): se quita la marca para que la
   orden se pueda volver a mandar. */
async function unmarkOrderImported(orderId) {
  const map = await getJsonSetting('qb_imported_orders');
  delete map[orderId];
  await saveSetting('qb_imported_orders', JSON.stringify(map));
}

/* Borra un Estimate / Invoice en QuickBooks. Si ya no existe (alguien lo
   borro a mano alla) regresa 'gone' en vez de fallar. */
async function deleteSalesDoc(type, id) {
  const entity = type === 'invoice' ? 'invoice' : 'estimate';
  let cur;
  try {
    cur = await qbFetch('/' + entity + '/' + encodeURIComponent(id));
  } catch (e) {
    if (/not found|object not found|6240|610/i.test(e.message)) return 'gone';
    throw e;
  }
  const doc = cur.Invoice || cur.Estimate;
  if (!doc || doc.status === 'Deleted') return 'gone';
  await qbFetch('/' + entity + '?operation=delete', { method: 'POST', body: { Id: String(doc.Id), SyncToken: doc.SyncToken } });
  return 'deleted';
}

/* ---- Adjuntar un archivo (foto) a un Estimate / Invoice (26/09/2026,
   el dueño: "cuando se suben a QuickBooks, puedes poner todas las fotos
   de esa orden?" -- y que le lleguen al cliente en el correo). Endpoint
   /upload de QuickBooks: multipart con los datos (JSON) y el archivo.
   IncludeOnSend = va adjunta cuando se manda el documento por correo. */
async function uploadAttachment(type, docId, fileName, buffer, contentType) {
  const [accessToken, realmId] = await Promise.all([getValidAccessToken(), getSetting('qb_realm_id')]);
  if (!realmId) throw new Error('QuickBooks is not connected yet.');
  const entity = type === 'invoice' ? 'Invoice' : 'Estimate';
  const boundary = '----gsms' + Date.now().toString(16) + Math.random().toString(16).slice(2);
  const meta = JSON.stringify({
    AttachableRef: [{ EntityRef: { type: entity, value: String(docId) }, IncludeOnSend: true }],
  ...
  const head = '--' + boundary + '\r\n' +
    ...
    'Content-Type: application/json\r\n\r\n' + meta + '\r\n' +
    '--' + boundary + '\r\n' +
    'Content-Disposition: form-data; name="file_content_01"; filename="' + fileName.replace(/"/g, '') + '"\r\n' +
    'Content-Type: ' + contentType + '\r\n\r\n';
  const body = Buffer.concat([Buffer.from(head, 'utf8'), buffer, Buffer.from('\r\n--' + boundary + '--\r\n', 'utf8')]);
  const res = await fetch(qbApiBase() + '/v3/company/' + realmId + '/upload?minorversion=75', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + accessToken, Accept: 'application/json', 'Content-Type': 'multipart/form-data; boundary=' + boundary },
    body
  });
  const data = await res.json().catch(() => ({}));
  const tid = res.headers.get('intuit_tid') || '';
  const fault = (data.Fault && data.Fault.Error && data.Fault.Error[0]) ||
    (data.AttachableResponse && data.AttachableResponse[0] && data.AttachableResponse[0].Fault && data.AttachableResponse[0].Fault.Error && data.AttachableResponse[0].Fault.Error[0]);
  if (!res.ok || fault) {
    const detail = (fault && (fault.Detail || fault.Message)) || ('HTTP ' + res.status);
    console.error('QuickBooks upload error', res.status, 'intuit_tid=' + tid, detail);
    throw new Error('QuickBooks upload error: ' + detail + (tid ? ' (intuit_tid ' + tid + ')' : ''));
  }
  return data;
}

/* Guarda datos extra de una orden ya mandada (p. ej. fotos adjuntas). */
async function updateImportedOrder(orderId, patch) {
  const map = await getJsonSetting('qb_imported_orders');
  if (!map[orderId]) return;
  map[orderId] = Object.assign({}, map[orderId], patch);
  await saveSetting('qb_imported_orders', JSON.stringify(map));
}

/* Cuales de estos documentos siguen existiendo en QuickBooks (25/09/2026,
   tab Sent: avisar si alguien lo borro directo alla). -> Set de Ids. */
async function existingDocIds(type, ids) {
  const entity = type === 'invoice' ? 'Invoice' : 'Estimate';
  const found = new Set();
  const clean = [...new Set((ids || []).map(String).filter(x => /^\d+$/.test(x)))];
  for (let i = 0; i < clean.length; i += 50) {
    const part = clean.slice(i, i + 50).map(x => "'" + x + "'").join(', ');
    const data = await qbQuery(`select Id from ${entity} where Id in (${part})`);
    ((data.QueryResponse && data.QueryResponse[entity]) || []).forEach(d => found.add(String(d.Id)));
  }
  return found;
}
