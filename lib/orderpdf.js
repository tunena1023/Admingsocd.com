...
function renderServicesTable(doc, order, services, history) {
  ...
  /* BUG REAL encontrado con una orden real (GS-1032-1001, 20/09/2026,
     ...
     en la lista original. El documento de finalizacion terminaba
     mostrando servicios de verdad completados como si siguieran
     "Added" sin mas. Ahora el estatus FINAL de la orden (o
     NotCompleted, que es aun mas especifico) siempre gana sin
     importar si el servicio es original o se agrego despues --
     'Added' solo aplica mientras la orden sigue en curso (ni
     Completed ni Cancelled todavia), igual que antes para una orden
     todavia activa o para el documento de una solicitud pendiente
     (Review). */
  const rowFor = s => {
