...
function buildOrderPdf(data) {
  ...
  /* --- Historial: nada se pierde a proposito, EXCEPTO ruido puramente
     operativo. A peticion del dueño (19/09/2026, con captura marcada):
       ...
       - "Order Assigned"/"Order Approved" -- estos SOLO se saltan
         cuando no traen ningun detalle real que mostrar (puro
         marcador de cambio de estatus, sin informacion propia -- el
         Status actual ya se ve arriba, en Order Details). Si SI traen
         detalle real (ej. un cambio de servicios real paso justo en
         ese mismo evento), se quedan igual que siempre. */
  const ALWAYS_HIDDEN = ['Document Generated', 'Document Failed', 'Archived', 'Batch Created'];
  const NOISY_IF_EMPTY = ['Order Assigned', 'Order Approved'];
