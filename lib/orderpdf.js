...
function buildOrderPdf(data) {
  ...
  /* --- Orden ---
     A peticion del dueño (19/09/2026): en 2 columnas -- una sola
     columna con puros campos cortos (fechas, estatus, etc.) dejaba
     mucho espacio vacio y se veia muy largo sin necesidad. */
  ...
  doc.kv2col([
    ...
    ['Submitted', fmtDateTime(order.createdDateTime)],
    ['Entry Date', fmtDate(order.EntryDate)],
  ...
  /* --- Servicios ---
     ...
     historial), no la lista actual -- un servicio quitado despues se
     queda en la tabla pero con Result='Removed', en vez de
     desaparecer sin dejar rastro. Lo que se AGREGO despues no entra a
     esta tabla (evita repetir el resto de la lista sin necesidad) --
     sale como un renglon corto aparte, justo debajo. Si el historial
     no trae ningun 'Services' (ordenes viejas sin ese registro),
     originalServices cae de vuelta a la lista actual tal cual, sin
     marcar nada como agregado/quitado -- no hay con que comparar. */
  doc.h2('Services');
  ...
  if (!originalServices.length) {
    ...
    const rows = originalServices.map(s => {
      const live = currentByKey[serviceKey(s)];
      let status;
      if (!live) {
        status = 'Removed';
    ...
    const originalKeys = {};
    originalServices.forEach(s => { originalKeys[serviceKey(s)] = true; });
    const addedSince = services.filter(s => !originalKeys[serviceKey(s)]);
    if (addedSince.length) {
      doc.gap(3);
      doc.text('Added since original order: ' + addedSince.map(serviceLabel).join(', '),
        { size: 9, color: [0.16, 0.42, 0.29] });
    }
  }
...
/* Reconstruye la lista ORIGINAL de servicios de la orden a partir del
   historial -- el primer renglon con FieldChanged='Services' que trae
   un NewValue valido (mas antiguo por fecha). Requiere que 'history'
   ya venga ordenado o no -- se ordena aqui de todos modos, no cuesta
   nada y no depende de que el llamador se acuerde de hacerlo.
   Si no hay ningun renglon de Services en el historial (ordenes viejas
   sin ese registro, o algun caso raro), regresa null -- el llamador
   entonces usa la lista actual tal cual, sin marcar nada como
   agregado/quitado (no hay con que comparar). */
function findOriginalServices(history) {
  const svcEntries = (history || [])
    .filter(h => clean(h.FieldChanged) === 'Services' && clean(h.NewValue))
    .map(h => ({
      date: h.ChangeDate || h.createdDateTime || '',
      payload: parseServicesPayload(h.NewValue)
    }))
    .filter(e => e.payload && Array.isArray(e.payload.services));
