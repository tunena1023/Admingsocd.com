...
async function readPackageContents(defaults) {
  ...
  const v = require('./settings-json').readJson(rows, PACKAGE_CONTENTS_KEY);
  if (v) return { map: v, row: true };
  return { map: JSON.parse(JSON.stringify(defaults || DEFAULT_PACKAGE_CONTENTS)), row: null };
}
