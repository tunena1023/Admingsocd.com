...
/* ===== Ascenso: 'Recurring Scheduled' -> 'Received' cuando llega el
   dia de la visita (DispatchDate <= hoy). A partir de ahi es una
   orden 'Received' normal para el resto del sistema.

   OJO -- pendiente real para la Fase 3 (Approvals simplificado), NO
   resuelto por esta funcion: admin.html ya tiene una regla existente
   (isFullyScheduled) que manda cualquier orden 'Received' con
   Supervisor+Fecha+Ventana ya puestos a ACTIVE (no Approvals), con un
   boton simple "Mark as Seen" -- no el confirmar/reasignar que se
   quiere aqui. Como una visita recurrente SIEMPRE nace asi de
   completa, sin ajustar approvalsOf() (y las otras 2 lineas que usan
   la misma condicion) para tratar las recurrentes como excepcion,
   esta funcion las ascenderia a 'Received' pero terminarian en Active
   con el boton equivocado, no en Approvals. */
async function promoteDueRecurringOrders(todayISO_) {
  ...
  for (const it of due) {
    ...
    try {
      await updateListItemByItemId(ORDERS_LIST, it.id, { Status: 'Received' });
